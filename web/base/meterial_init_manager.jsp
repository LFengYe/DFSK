        ...
        <script type="text/javascript">
            window.onload = function () {
                ...
                $("#importData").bind("click", function () {
                    parent.displayLayer("./base/material_init_import.jsp", "导入原材料期初", function(){getBaseInfoList("search_form");});
                });
            ...
            function getBaseInfoList(formId) {
                ...
                getDataInterface('materialInitGet.do', sendBody, function(result) {
                    var obj = JSON.parse(result);
                    if (obj.status === 0) {
                        var htmlStr = "<table class='detail_data'><tr class='table_header'><td>序号</td><td>件号名称</td><td>件号</td><td>车型</td><td>期初数量</td><td>期初日期</td><td>操作</td></tr>";
                        for (var i = 0; i < obj.data.length; i++) {
                            htmlStr += "<tr class='table_content'>";
                            htmlStr += "<td>" + (obj.data[i].materialInitId ? obj.data[i].materialInitId : "") + "</td>";
                            ...
                            htmlStr += "<td>" + (obj.data[i].initNum ? obj.data[i].initNum : "") + "</td>";
                            htmlStr += "<td>" + (obj.data[i].initDate ? obj.data[i].initDate : "") + "</td>";
                            htmlStr += "<td>";
        ...
        <div>
            <div id="message_list">
                <table class='detail_data'>
                    <tr class='table_header'><td>序号</td><td>件号名称</td><td>件号</td><td>车型</td><td>期初数量</td><td>期初日期</td><td>操作</td></tr>
                </table>
            </div>
