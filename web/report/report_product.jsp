        ...
        <script type="text/javascript">
            window.onload = function () {
                ...
                $("#export").bind("click", function () {
                    ...
                    getDataInterface('exportProductReport.do', sendBody, function (result) {
                        var obj = JSON.parse(result);
                        if (obj.status === 0) {
                            location.href = encodeURI(obj.data);
//                            location.href = encodeURI('../exportFile/' + obj.data);
//                            window.open(encodeURI('exportFile/' + obj.data));
                        }
                        alert(obj.message);
                    });
                });
            ...
            function getList(formId) {
                ...
                getDataInterface('reportWithTime.do', sendBody, function (result) {
                    var obj = JSON.parse(result);
                    if (obj.status === 0) {
                        var htmlStr = "<table class='detail_data'><tr class='report_table_header'><td>月预示计划下达数量</td><td>件号</td><td>件号名称</td><td>期初成品库存</td><td>当日入库</td><td>累计入库</td><td>当日出库</td><td>累计出库</td><td>库存</td></tr>";
...
//                            htmlStr += "<td>" + obj.data[i].materialInToday + "</td>";
                            htmlStr += "<td>" + obj.data[i].productInToday + "</td>";
//                            htmlStr += "<td>" + obj.data[i].materialInCount + "</td>";
                            htmlStr += "<td>" + obj.data[i].productInCount + "</td>";
                            htmlStr += "<td>" + obj.data[i].productOutToday + "</td>";
                            htmlStr += "<td>" + obj.data[i].productOutCount + "</td>";
//                            htmlStr += "<td>" + obj.data[i].materialStock + "</td>";
                            htmlStr += "<td>" + obj.data[i].productStock + "</td>";
                            htmlStr += "</tr>";
                        }
                        htmlStr += "</table>";
                        ...
                        var htmlStr = "<table class='detail_data'><tr class='report_table_header'><td>月预示计划下达数量</td><td>件号</td><td>件号名称</td><td>期初成品库存</td><td>当日入库</td><td>累计入库</td><td>当日出库</td><td>累计出库</td><td>库存</td></tr>";
                        htmlStr += "<td colspan='9'><h3>没有查询到相关数据！</h3></td>";
                        htmlStr + "<tr></tr></table>";
                        $("#message_list").html(htmlStr);
                        layerPage(0);
                    }
